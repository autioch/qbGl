...
(function(qb) {
  const scene = new qb.Scene('webgl11');
  ...
  const textureMoon = new qb.Texture(context, {
    url: 'moon.gif',
  ...
  const view = new qb.View('#container11');

  let lastMouseX = null;
  let lastMouseY = null;
  const light = new qb.Light(context);

  const moonRotationMatrix = new qb.Matrix();
  ...
  view.onmousemove = function(event) {
    ...
    const newRotationMatrix = new qb.Matrix();
  ...
  const bufferStash = new qb.Shape(context);
