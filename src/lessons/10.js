...
(function(qb) {
  const scene = new qb.Scene('webgl10');
  ...
  const textureWall = new qb.Texture(context, {
    url: 'wall.jpg'
  });
  const textureFloor = new qb.Texture(context, {
    url: 'podloga.jpg'
  });
  const textureCeiling = new qb.Texture(context, {
    url: 'ceiling.jpg'
  });
  ...
  const wallsShape = new qb.Shape(context);
  const floorShape = new qb.Shape(context);
  const ceilingShape = new qb.Shape(context);

  const view = new qb.View('#container10');
  ...
  scene.update = function(elapsedTime) {
    if (speed !== 0) {
      xPos -= Math.sin(qb.degToRad(yaw)) * speed * elapsedTime;
      zPos -= Math.cos(qb.degToRad(yaw)) * speed * elapsedTime;
      joggingAngle += elapsedTime * 0.6; // 0.6 "fiddle factor" - makes it feel more realistic :-)
      yPos = Math.sin(qb.degToRad(joggingAngle)) / 20 + 0.4;
    }
