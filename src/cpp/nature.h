...
void branch::grow_leaves() {
  ...
  if (leaf_num < 2)
    leaf_num = 0;
  if (leaf_num > branch_leaf)
    leaf_num = branch_leaf;
...
void tree::paint(bool b, bool leaves) { branches[0].paint(b, leaves); };
...
void butterfly::move() {
  if (((abso(end[0] - pos[0]) <= abso(mov[0]))) ||
...
int WriteBitmapFile(char *filename, int width, int height,
  ...
  bitmapInfoHeader.biSizeImage =
      width * abs(height) * 3; // width * height * (bajty RGB)
